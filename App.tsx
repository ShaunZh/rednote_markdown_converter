...
const App: React.FC = () => {
  // State
  const [markdown, setMarkdown] = useState<string>(`# Welcome to RedNote Converter 📕

Transform your Markdown into beautiful 3:4 images perfect for **Xiaohongshu**.

## Features

- 🎨 **Multiple Themes**: Minimalist, Dark, Morandi
- 💻 **Code Highlighting**: With Mac-style headers
- 📱 **Smart Pagination**: Automatically splits content into cards
- ⚡️ **Instant Preview**: Real-time rendering
...
2. Select a theme on the right
3. Click **Export Images** to download a ZIP

\`\`\`javascript
// Example Code Block
const createMagic = () => {
  console.log("Hello RedNote!");
  return true;
}
\`\`\`
...
Enjoy creating!`);
  ...
  // Parse blocks
  const blocks = React.useMemo(() => splitMarkdownIntoBlocks(markdown), [markdown]);
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
        ...
        const paddingVal = parseInt(currentTheme.container.padding) || 24;
        const paddingY = paddingVal * 2; 
        const MAX_CONTENT_HEIGHT = CARD_HEIGHT - paddingY - 40; // -40 for footer
        ...
        setPages(newPages);
    }, 150); // Slightly increased delay for style application

    return () => clearTimeout(timeout);
  }, [blocks, currentTheme]); // Re-run when theme changes because styles/fonts affect height
  ...
  return (
    <div className="flex flex-col h-screen bg-neutral-100 overflow-hidden text-slate-800">
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[350px] border-r border-neutral-200 bg-white flex flex-col shrink-0 relative z-30">
          {/* Tab Navigation */}
        ...
        <div className="flex-1 bg-neutral-100/50 overflow-y-auto p-8 relative flex flex-col items-center gap-8 shadow-inner">
      ...
      <ImportModal 
        ...
        onImport={(content) => {
          setMarkdown(content);
          // Optional: Switch to editor tab if not active, or just flash success
        }} 
      />
